...
from app.schemas.v2.face import (
    BulkEnrollItem,
    BulkEnrollResponse,
    BulkImportRequest,
    ClearAllResponse,
...
async def enroll_face(
    ...
    return FaceEnrollResponse(
        ...
        model_version=face_service.model_version if hasattr(face_service, "model_version") else None,
    ...
    if liveness_result["status"] != "passed":
        ...
        return FaceRecognizeResponse(
            ...
            liveness=LivenessInfo(
                score=liveness_result.get("liveness_score"),
                label=liveness_result.get("label"),
                components=liveness_result.get("components"),
            ),
            processing_time_ms=elapsed_ms,
            model_version=face_service.model_version if hasattr(face_service, "model_version") else None,
...
@router.get("/persons", response_model=PaginatedPersonsResponse)
...
async def get_import_status(
    ...
    if job.completed_at and job.created_at:
        processing_time = round(
            (job.completed_at - job.created_at).total_seconds() * 1000, 2
        )
